...
export interface IUserCardPropiedades {
  user: IUsuario;
}

const UserCard: React.FC<IUserCardPropiedades> = ({ user }) => {
  return (
    <div className='flex flex-row gap-4 border border-gray-300 rounded-lg overflow-hidden w-96'>
      <div className='aspect-square w-32'>
        <Image width={128} height={128} className='object-cover w-full h-full' src={user.picture} alt={`Foto de ${user.firstName} ${user.lastName}`} />
      </div>
